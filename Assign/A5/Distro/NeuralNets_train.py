...
##########################################################
	...
	#No Hidden Layers	
	if(NeuralNets_global_data.N_hidden < 1):
		# print "Entered No Hidden Layer If Statement inside FeedForward"
		
		for j in range(NeuralNets_global_data.N_out): 
			summ = 0
			for i in range(NeuralNets_global_data.N_in):
				summ = summ + input_sample[i]*NeuralNets_global_data.W_io[i][j]
			#summ = summ + input_sample[-1] #add the bias term
			outputActivation[j] = sigmoid(summ)

	else:
		for h in range(NeuralNets_global_data.N_hidden - 1):
   			summ = 0
   			for i in range(NeuralNets_global_data.N_in):
 	               		summ += input_sample[i] * NeuralNets_global_data.W_ih[i][h]
            		hiddenActivation[h] = sigmoid(summ)
    	...
    	for j in range(NeuralNets_global_data.N_out):
            	...
            	for h in range(NeuralNets_global_data.N_hidden):
                	summ += hiddenActivation[h] * NeuralNets_global_data.W_ho[h][j]
            	outputActivation[j] = sigmoid(summ)
...
################################################################
	...
	hid_err=zeros(shape=(NeuralNets_global_data.N_hidden,1))
	...
	eh = zeros(shape=(NeuralNets_global_data.N_hidden, 1))

	oA, hA = FeedForward(input_sample)

	if NeuralNets_global_data.sig_type:
		# Target for Hyperbolic Tangent function 
		t = [0.6, -0.6]
	else:
		# Target for logistics function
		t = [0.8, 0.2]


	for j in range(NeuralNets_global_data.N_out):
                if(input_label == j):
                        errors[j] = t[0] - outputActivation[j]
                else:
                        errors[j] = t[1] - outputActivation[j]

	
	output_deltas = [0.0] * NeuralNets_global_data.N_out
	hidden_deltas = [0.0] * NeuralNets_global_data.N_hidden

	if (NeuralNets_global_data.N_hidden == 0):
		for j in range(NeuralNets_global_data.N_out):
			output_deltas[j] = sigmoid_prime(outputActivation[j]) * errors[j]
			for i in range(NeuralNets_global_data.N_in):
				NeuralNets_global_data.W_io[i][j] += NeuralNets_global_data.alpha * output_deltas[j] * input_sample[i]
		...
		for h in range(NeuralNets_global_data.N_hidden):
			summ = 0
			for j in range(NeuralNets_global_data.N_out):
				summ += NeuralNets_global_data.W_ho[h][j] * sigmoid_prime(outputActivation[j]) * errors[j]    
			hid_err[h] = summ 

		for j in range(NeuralNets_global_data.N_out):
			output_deltas[j] = sigmoid_prime(outputActivation[j]) * errors[j]
			for h in range(NeuralNets_global_data.N_hidden):
				NeuralNets_global_data.W_ho[h][j] += NeuralNets_global_data.alpha * output_deltas[j] * hiddenActivation[h]

		for h in range(NeuralNets_global_data.N_hidden - 1):
			for i in range(NeuralNets_global_data.N_in):
				NeuralNets_global_data.W_ih[i][h] += NeuralNets_global_data.alpha * sigmoid_prime(hiddenActivation[h]) * hid_err[h] * input_sample[i]            
	return(errors)
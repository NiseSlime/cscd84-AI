...
def MiniMax_utility(GameState, StateTracker, agent_id, depth,alpha,beta):
  ############################################################
  ...
  # Inputs:
  #
  # GameState, StateTracker, agent_id, depth - see the 
  ...
  if not (depth>MiniMax_global_data.search_END or checkForCats(GameState) or checkForCheese(GameState)):
	[pl,utility]=MiniMax_search(GameState,StateTracker,agent_id+1,depth+1,alpha,beta)
  ...
  dist_cat = 100000000
  while index <= MiniMax_global_data.Ncats:
    dist = (abs(GameState[index][0] - GameState[0][0]) + abs(GameState[index][1] - GameState[0][1]))
    if dist < dist_cat:
      dist_cat = dist
    index+=1
  # calculate distance to nearest cheese 
  dist_cheese = 100000000
  while index < len(MiniMax_global_data.GameState):
    dist = (abs(GameState[index][0] - GameState[0][0]) + abs(GameState[index][1] - GameState[0][1]))
    if dist < dist_cheese:
      dist_cheese = dist
    index+=1
  ...
  # if dist_cat < 5:
  #   mult = dist_cat
  # dist_cheese = abs(GameState[2][0] - GameState[0][0]) + abs(GameState[2][1] - GameState[0][1])
  # dist_cat = abs(GameState[1][0] - GameState[0][0]) + abs(GameState[1][1] - GameState[0][1])
  # if dist_cat < 5:
  #   utility = (10000-dist_cheese)-(10000-dist_cat)
  # else:
  #   utility = 10000-dist_cheese
  #utility = dist_cat-dist_cheese
  utility = (10000-dist_cheese)-(10000-dist_cat)
  #print "utility given by function is " + str(utility)
  return [pl,utility]	# Of course, by now your utility should be non-zero

def MiniMax_search(GameState, StateTracker, agent_id, depth, alpha, beta):
  ###############################################################
  ##
  ...
  ## StateTracker - A list of configurations explored already up the
  ##                tree. Used to avoid inserting duplicate
  ##                configurations on the same subtree (different
  ##                subtrees may still contain identical configurations
  ...
  # Remember that we have a search tree, so we have to ensure no loops are created by
  # re-visiting nodes further up the search tree. We use the 'StateTracker' list of
  # visited game states for this. Notice that this is not a global list. We only
  # care about not visiting the same state if it has been visited on the same subtree
  # that contains *THIS* state. It is still possible that a given configuration
  # will be explored multiple times on different subtrees.
  #
  # This is in fact ESSENTIAL for MiniMax to work properly. Why is that?
  
  # Don't mess with the StateTracker!
  StateTracker_local=list(StateTracker)   # Create a local copy of the state list above
  StateTracker.append(GameState)	  # insert the current state

  if debug:
    print "MiniMax search called for agent ",agent_id, "\n", "Mouse: ",GameState[0],", Cat:",GameState[1]
    #print "List of explored states"
    #print "*************************"
    #print StateTracker_local
    #print "*************************"
  ...
  if (MiniMax_global_data.A[x+(y*MiniMax_global_data.msx)][3]==1):
   ...
   if (not (GameState_t in StateTracker_local)):
  ...
  opt_idx=0		# Use this to record the *INDEX* of the winning successor
	...
	# Add all your code in this space
  counter = 0
  # tempGS = list(GameState)
  # tempST = list(StateTracker_local)
  #tempST.append(tempGS)
  while counter < len(successors):
    tempGS = list(GameState)
    tempST = list(StateTracker_local)
    tempGS[agent_id] = successors[counter]
    tempST.append(tempGS)
    util = MiniMax_utility(tempGS,tempST, agent_id, depth,alpha,beta)
    #print "utility captured in util is " + str(util)
    if (not((util[0] == [-1,-1]) and (util[1] == 0))):
      utilities.append(util[1])
      pls.append(util[0])
      if MiniMax_global_data.SearchType == 2:
        if agent_id == 0: # THIS IS MAX NODE
          alpha = max(alpha,util[1])
          if beta <= alpha:
            break
        else: # This is min node
          beta = min(beta, util[1])
          if beta <= alpha:
            break
    counter += 1
  ...
  # print "agent id is: " + str(agent_id)
  # print "utilities are " + str(utilities)

  # print "optimum is " + str(opt_idx)
  ################################################################
  ## 
